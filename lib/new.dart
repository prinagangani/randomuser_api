...
class _demoState extends State<demo> {
  ...
  Future<void> updata() async {
    ...
    nn = updataa.fromJson(result);
    print("====${nn!.results![0].id!.name}");
    statas = true;
  }

  @override
  Widget build(BuildContext context) {
    return statas?Scaffold(
      body: ListView.builder(
        itemCount: nn!.results!.length,
        itemBuilder: (context, index) {
          return ListTile(
            title: Text("${nn!.results![index].name!.title}"),
          );
        },
      ),
...
class updataa {
  ...
  updataa({this.results, this.info});

  updataa.fromJson(Map<String, dynamic> json) {
